  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1fr_320px]">
      ...
      <div className="space-y-8">
        ...
        {heroArticle && (
          <section className="group relative overflow-hidden rounded-3xl border border-slate-900 bg-slate-950/40 p-5 sm:p-6 backdrop-blur-md hover:border-emerald-500/20 transition-all duration-300">
            <div className="grid gap-6 md:grid-cols-2 items-center">
              ...
              <Link href={`/maqola/${heroArticle.slug}`} className="relative block overflow-hidden rounded-2xl aspect-video md:aspect-[4/3] w-full bg-slate-950">
                ...
                <span className="absolute left-4 top-4 rounded-full bg-emerald-500 px-3 py-1 text-[10px] font-extrabold uppercase tracking-widest text-slate-950">
                  Asosiy Mavzu
                </span>
              </Link>
              ...
              <div className="flex flex-col h-full justify-center">
                <div className="mb-3.5 flex items-center gap-3 text-xs">
                  <span className="rounded-full bg-emerald-500/10 px-3 py-0.5 font-bold text-[10px] uppercase tracking-wider text-emerald-400 border border-emerald-500/10">
                ...
                <Link href={`/maqola/${heroArticle.slug}`}>
                  <h1 className="mb-4 text-xl sm:text-2xl font-extrabold leading-tight text-white hover:text-emerald-400 transition-colors duration-200">
                    {heroArticle.title}
                  </h1>
                </Link>
                ...
                <div className="flex items-center justify-between border-t border-slate-900/60 pt-4">
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500 px-5 py-2 text-xs font-bold text-slate-950 hover:bg-emerald-400 hover:scale-[1.02] active:scale-[0.98] transition-all"
                  >
                    O&apos;qish
        ...
        <section>
          <div className="mb-6 flex items-center justify-between border-b border-slate-900 pb-3">
          ...
          {hasContent ? (
            ...
            <div className="rounded-2xl border border-slate-950 bg-slate-950/20 p-10 text-center leading-relaxed text-slate-500">
      ...
      <aside className="space-y-8">
        ...
        {(digest || []).length > 0 && (
          <section className="rounded-2xl border border-slate-900 bg-slate-950/40 p-5 backdrop-blur-md">
            <div className="mb-4 flex items-center gap-2 border-b border-slate-900 pb-2">
            ...
            <ul className="space-y-3">
              {digest.map((article, idx) => (
                <li key={article.id} className="group flex items-start gap-2.5 text-xs font-medium leading-relaxed">
                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-emerald-500/10 text-[10px] font-bold text-emerald-400 border border-emerald-500/10">
                    {idx + 1}
                  </span>
            ...
            {(top || []).length > 0 ? (
              top.map((article, index) => (
                <Link
                  ...
                  className="group flex items-start gap-3 text-xs hover:bg-slate-900/10 p-1.5 rounded-lg transition-colors"
                >
                  <span className="text-xl font-extrabold text-slate-800 group-hover:text-emerald-500/60 transition-colors leading-none w-6 shrink-0 text-center">
                    0{index + 1}
                  </span>
              ...
              <p className="text-xs text-slate-500">Hali yangiliklar yo&apos;q.</p>
            )}
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-slate-900 bg-slate-950/40 p-5 backdrop-blur-md">
          ...
          <div className="flex flex-wrap gap-2">
            {(trends || []).length > 0 ? (
              ...
              <p className="text-xs text-slate-500">Mavzular shakllanmagan.</p>
            )}
          </div>
        </section>
      </aside>
